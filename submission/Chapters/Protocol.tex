...
\begin{algorithm}[H]
  ...
  \KwResult{Session ID construction in the context of party $\hat{X}$}
  \Begin{	
	$c_{\hat{X}} \leftarrow random$
	
	$Broadcast(c_{\hat{X}})$
	
	$Outstanding \leftarrow P \textbackslash \{\hat{X}\}$
...
\begin{algorithm}[H]
  \KwIn{$\mathcal{P}$ : participants list, $sid$: the session ID, $\mathcal{S}$: the association table}
  \KwOut{$\mathcal{A}$: "OK" if verification went good, "Error" if it went wrong}
  \KwResult{authenticate previously unauthenticated protocol parameters for the current session in the context of party $\hat{X}$}
  \Begin{	
