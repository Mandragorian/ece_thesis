...
\begin{algorithm}[H]
  \KwIn{$M$ : message}
  \KwExtraIn {$\mathcal{S}$ : the ephemeral signing key assocation table}
...
\begin{algorithm}[H]
  \KwIn{$\mathcal{S}$ : the ephemeral signing key assocation table}
...
\begin{algorithm}[H]
  ...
  \caption{GKA($sid$, $\mathcal{S}$, $\mathcal{P}$) - executes a Group Key Agreement and produces the shared secret in the context of party $\hat{X}$.}
...
\begin{algorithm}[H]
  ...
  {	
    ...
    \While{$Outstanding \neq \emptyset$}
    {
      $(\hat{Y}, M_{\hat{Y}}) := AuthReceive(\mathcal{S})$
