...
\vfill
\begin{algorithm}[H]
  \KwIn{$\mathcal{P}$s : participants list}
  ...
  \Begin
  {
    ...
    \If{$\hat{Y}_{prev}$ = NULL}{
      $SendUpflow(P, [G], x, \hat{Y}_{next})$
    }
    \Else
    {
      $(\hat{Y}, key\_list) \leftarrow AuthReceive()$

      \If{$\hat{Y}$ $\neq$ $\hat{Y}_{prev}$ $\lor$ Validate($key\_list$) = false}
      {
        \Return{error}
      }
      \If{$\hat{Y}_{next}$ $\neq$ NULL}
    ...
    \If{$\hat{Y}$ $\neq$ $\mathcal{P}.Last()$ $\lor$ Validate($key\_list$) = false}
    {
      \Return{error}
    }
